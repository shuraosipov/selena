...
include 'paged_status.php';


echo <<<HEREDOC
<!-- Открытие контейнера main -->


<!-- Заголовок с пояснением в верху страницы -->
<p id="titlereception">	Обращения со статусом $type</p>
...
{
	echo "<a id=\"first_page\" href=\"?page=1&status=$status&type=$type\">На первую</a>";
	for ($x=$left_neighbor;$x<=$right_neighbor;$x++)
	{
		...
		{
			echo "<span id=\"pages\"><a href=\"?page=$x&status=$status&type=$type\">$x</a></span>"." ";
		}
	} 
	echo "<a id=\"last_page\" href=\"?page=$pages&status=$status&type=$type\">На последнюю ($pages)</a>";
}
echo "</div>";
